...
import { Loader2, DollarSign, CheckCircle, Clock, Star, TrendingUp, Send, Download, X, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const saveNotes = async () => {
    ...
    setSavingNotes(true);
    const updated = await base44.entities.Job.update(selectedJob.id, {
      admin_notes: adminNotes,
      admin_mpesa_reference: adminMpesa || undefined,
    });
  ...
  const exportCSV = () => {
    ...
    const rows = jobs.map(j => [
      ...
      j.location_description, j.member_notes || "",
      j.status,
            ...
            <>
              ...
              <div className="flex gap-2 mb-4">
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                  <SelectTrigger className="flex-1 text-sm h-9"><SelectValue placeholder="All statuses" /></SelectTrigger>
                  <SelectContent>
                ...
                <Select value={filterPayment} onValueChange={setFilterPayment}>
                  <SelectTrigger className="flex-1 text-sm h-9"><SelectValue placeholder="All payments" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">All Payments</SelectItem>
                    <SelectItem value="unpaid">Unpaid</SelectItem>
                    <SelectItem value="paid_to_waiyaki">Paid to Waiyaki</SelectItem>
                    <SelectItem value="confirmed">Confirmed</SelectItem>
                    <SelectItem value="mechanic_paid">Mechanic Paid</SelectItem>
                  </SelectContent>
                </Select>
              </div>
                      ...
                      <div className="flex gap-2 items-center">
                        {job.payment_status === "paid_to_waiyaki" && (
                          <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white text-xs"
                            ...
                            {updating === job.id + "_confirm" ? <Loader2 className="w-3 h-3 animate-spin" /> : "Confirm Payment"}
                          </Button>
                        )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea
                  className="w-full border border-gray-200 rounded-lg p-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-amber-400"
                  rows={3}
              ...
              <div className="flex gap-2">
                ...
                {selectedJob.mechanic_payment_sent && (
                  <div className="flex-1 text-center text-green-600 font-bold py-2">✅ Prince Waiyaki Paid</div>
                )}
              </div>
