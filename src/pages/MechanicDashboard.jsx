...
export default function MechanicDashboard() {
  ...
  const [view, setView] = useState("jobs"); // jobs | earnings
                ...
                {todayJobs.map(job => (
                  <div key={job.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{job.member_name}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-green-700">KES {Math.round((job.price_kes || 0) * 0.8).toLocaleString()}</p>
      ...
      {view === "earnings" && !loading && (
        <div className="space-y-4">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
            ...
            <p>You receive <strong>80% of every job fee</strong>. Tex Wambui (Waiyaki House) pays you via M-PESA within 24 hours of every confirmed job.</p>
            <p className="mt-2">Waiyaki House keeps 20% as the coordination fee.</p>
          </div>
          <div>
            ...
            <div className="space-y-2">
              {completed.slice(0,20).map(job => (
                <div key={job.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm flex justify-between items-center">
                  <div>
                    <p className="font-semibold text-gray-900 text-sm">{job.service_type_name}</p>
                    <p className="text-xs text-gray-500">{job.member_name} · {format(new Date(job.created_date), "dd MMM")}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-black text-green-700">KES {Math.round((job.price_kes || 0) * 0.8).toLocaleString()}</p>
